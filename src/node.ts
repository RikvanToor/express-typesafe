...
export type Response<C,R> = List.Includes<ValidStatusCodes, C> extends 1 ? [C, R] : never

export type Handler<Rs extends any[]> = {
  0: List.Head<Rs> extends Response<infer C, infer R>
    ? (List.Head<Rs> | Handler<List.Tail<Rs>>)
    : never
  1: never
}[List.Length<Rs> extends 0 ? 1 : 0]
...
type _Flatten<Ns extends any[]> = {
  ...
  : (
    List.Head<Ns> extends Capture<infer T, infer n>
      ? (t: T) => _Flatten<List.Tail<Ns>>
      : (
        List.Head<Ns> extends Body<infer T>
        ? (t: T) => _Flatten<List.Tail<Ns>>
        : (
          List.Head<Ns> extends Handler<infer Rs>
          ? List.Head<Ns>
          : never
        )
      )
...
// Would like to forbid this. Don't know how.
function unsafeHandler(): Handler<[any]> {
  return "test";
}
